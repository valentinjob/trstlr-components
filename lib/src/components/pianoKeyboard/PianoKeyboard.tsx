...
import {PianoKeyboardProps, KeyImpactProviderProps, KeyProps} from './types';

const BlackKey: FC<KeyProps> = ({name, pressedBy, containerStyle}) => {
  ...
  return <TouchableOpacity style={keyStyle} activeOpacity={0.8} />;
};

// eslint-disable-next-line @typescript-eslint/no-unused-vars
const WhiteKey: FC<KeyProps> = ({name, pressedBy, containerStyle}) => {
...
class KeyImpactProvider extends React.Component<KeyImpactProviderProps> {
  ...
  render() {
    const childrenWithPressedBy = React.Children.map(
      this.props.children,
      child =>
        // @ts-ignore
  ...
  return (
    <ScrollView horizontal={true} scrollEnabled={scrollEnabled}>
      <View style={{...styles.container, ...containerStyle}}>
        <KeyImpactProvider pressedKeys={pressedKeys}>
          <WhiteKey name={'C0'} />
